...
public class LoginControl extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // luu username, password vao session
        ...
        if (nvDao.kiemTraDangNhap(nv)) {
            // luu lai thong tin nguoi dung duoc update
            session.setAttribute("nhanVien", nv);

            // xoa thong tin về username, password, loginError
            session.removeAttribute("username");
            session.removeAttribute("password");
            session.removeAttribute("loginError");
            if (nv.getVaiTro().equalsIgnoreCase("quanly")) {
                // chuyen huong sang trang cua quan ly
                response.sendRedirect("home-manager");
            } else {
                // chuyen huong sang trang cua nhan vien
            }
        } else {
